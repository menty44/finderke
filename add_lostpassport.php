...
$v6=$_REQUEST['contact'];
	$v7=$_REQUEST['reward'];
          ...
          {
           $i=mysql_query("select * from kenyan_passport where passport_no=$v1",$con);
           $check='';
                  while($row = mysql_fetch_array($i))
                    {
 
                          $check=$row['passport_no'];
                   ...
                   if($check==NULL)
                  {
                        ...
                        $q="insert into kenyan_passport values('$v1','$v2','$v3','$v4','$v5','$v6','$v7')";
                        $s= mysql_query($q);
                        if(!$s)
                          {
